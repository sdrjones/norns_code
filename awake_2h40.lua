...
-- llllllll.co/t/21022
--
-- mods for 2 x 40h grids by steve
...
function gridredraw()
  ...
  if edit_ch == 1 then
    for x = 1, 16 do
      if one.data[x] > 0 then led(x, 9-one.data[x], 15) end
    end
    if one.pos > 0 and one.data[one.pos] > 0 then
      led(one.pos, 9-one.data[one.pos], (pulse_update==1) and 0 or 15)
    else
      led(one.pos, 1, (pulse_update==0) and 0 or 15)
    end
  end
  if edit_ch == 2 then
    for x = 1, 16 do
      if two.data[x] > 0 then led(x, 9-two.data[x], 15) end
    end
    if two.pos > 0 and two.data[two.pos] > 0 then
      led(two.pos, 9-two.data[two.pos], (pulse_update==1) and 0 or 15)
    else
      led(two.pos, 1, (pulse_update==0) and 0 or 15)
    end
  end
  refresh()
end
